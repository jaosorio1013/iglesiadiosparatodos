<footer id="footer-1" class="footer" style="background: #f2f2f2">
	<div class="container">
		<div class="row">
			<div class="col-md-6 col-md-offset-3">
				<div class="widget about-us text-center">
					{{--<img src="/assets/img/logo.png" class="hidden-xs" height="90px"/>--}}
					<img src="/assets/img/logo.png" height="90px"/>
					{{--<p>Somos una iglesia de brazos abiertos, de amor incondicional, de pasión inextinguible y en absoluta dependencia del Único Dios verdadero revelado en Jesucristo.</p>--}}
				</div>
				<!-- /.about-us -->
			</div>
			<!-- /.col-md-5 -->
			{{--<div class="col-md-4">--}}
				{{--<div class="widget popular-posts">--}}
					{{--<h5 class="subpage-title">Posts populares</h5>--}}
					{{--<ul class="recent-posts">--}}
						{{--<li>--}}
							{{--<img src="/assets/img/blog/recent-posts/photo (1).jpg" alt="Post Image">--}}
							{{--<h5>--}}
								{{--<a href="post-1.html">The Ultimate Guide to Adjustment Layers – Vibrance</a>--}}
								{{--<small>Posted July 20, 2013</small>--}}
							{{--</h5>--}}
						{{--</li>--}}
					{{--</ul>--}}
				{{--</div>--}}
				{{--<!-- /.popular-posts -->--}}
			{{--</div>--}}
			<!-- /.col-md-4 -->
			{{--<div class="col-md-3">--}}
				{{--<div class="widget flickr-photos">--}}
					{{--<h5 class="subpage-title">Fotos en Flickr</h5>--}}
					{{--<ul class="flickr-photos-list"></ul>--}}
				{{--</div>--}}
				{{--<!-- /.flickr-photos -->--}}
			{{--</div>--}}
			<!-- /.col-md-3 -->
		</div>
		<!-- /.row -->
	</div>
	<!-- /.container -->
</footer>
<!-- /#footer-1 -->
<footer id="footer-2" class="footer">
	<div class="container">
		<div class="row">
			<div class="col-xs-12 col-sm-6 footer-info-wrapper">
				<span>
                    Iglesia Dios Para Todos © 2015.
                    {{--Powered by <a href="https://github.com/jaosorio1013" target="_blank">Jaosorio1013</a>--}}
                </span>
			</div>
			<!-- /.footer-info-wrapper -->
			<div class="col-xs-12 col-sm-6 social-link-wrapper">
				<ul class="social-links">
					<li><a href="https://www.facebook.com/iglesiadiosparatodos" target="_blank"><i class="icon-facebook"></i></a></li>
					{{--<li><a href="#"><i class="icon-google-plus"></i></a></li>
					<li><a href="#"><i class="icon-twitter"></i></a></li>
					<li><a href="#"><i class="icon-pinterest"></i></a></li>
					<li><a href="#"><i class="icon-linkedin"></i></a></li>
					<li><a href="#"><i class="icon-dribbble"></i></a></li>--}}
				</ul>
			</div>
			<!-- /.social-link-wrapper -->
		</div>
		<!-- /.row -->
	</div>
	<!-- /.container -->
</footer>
<!-- /#footer-2 -->